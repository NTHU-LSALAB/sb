        ...
        with tempdir() as judgedir:
            ...
            if p.returncode != 0:
                return Result(
                    ...
                    f'inner non zero exit status: {p.returncode} {p.stderr.decode("ascii")} {p.stdout.decode("ascii")}'
                )
            try:
...
class HW4Runner(SbatchRunner):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def sbatch_options(self):
        return ['-ppp', '-N1', '-n1', '-c1', '--gres=gpu:1', '-whades05']
    ...
    def prepare_inner(self):
        ...
        shutil.copy(f'/home/ipc20/ta/hw4/testcase/{self.case}/case{self.case}.in', self.judgein)
    ...
    def validate(self, output_file, time):
        valid = self.judgein
        ret = subprocess.run(['/home/ipc20/ta/hw4/judge/validation', valid, self.judgeout], stdout=subprocess.DEVNULL)
        if ret.returncode != 0:
            return Result(False, time, Verdict.WA, 'wrong nonce')    
            # raise ValidationError(f'wrong nonce')
