...
func parseOptions(remoteClient bool) *judge.Options {
	opt := &judge.Options{}
	...
	if remoteClient {
		fs.StringVar(&opt.AsUser, "as", currentUser.Username, "Run the judge as the user. Should be verified by an SSH connection.")
	} else {
	...
	if remoteClient {
		// fs.IntVar(&opt.SSHAuthPort, "authPort", 22, "SSH authentication port for remote client.")
		opt.SSHAuthPort = sb.SSHPort
		opt.Server = sb.DefaultAddr
		fs.StringVar(&opt.SSHKeyPath, "sshkey", "~/.ssh/id_rsa", "SSH authentication key location for remote client.")
	}else {
...
func main() {
	remoteClient := false
	options := parseOptions(remoteClient)
	judge.MainOptions(options, remoteClient)
}
